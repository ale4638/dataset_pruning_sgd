...
def calc_s_test(model, test_loader, train_loader, save=False, gpu=-1,
    ...
    for i in range(start, len(test_loader.dataset)):
        ...
        s_test_vec = calc_s_test_single_icml(model, z_test, t_test, train_loader,
...
#liang's method
def calc_s_test_single_icml(model, z_test, t_test, all_data, gpu=0,
    ...
    s_test_vec_list = []
    for i in range(r):
        s_test_vec_list.append(-torch.nn.utils.parameters_to_vector(tuple(s_test(z_test, t_test, model, all_data,
                                      gpu=gpu, damp=damp, scale=scale,
                                      recursion_depth=recursion_depth))).cpu().numpy())
    ...
    ################################
    s_test_vec = np.array(s_test_vec_list).mean(0)

    return s_test_vec

#Using SGD
def calc_s_test_sgd(model,all_data, gpu=0,
                       damp=0.01, scale=25,r=1,target_epoch = 20):
    """Calculates s_test for a single test image taking into account the whole
    training dataset. s_test = invHessian * nabla(Loss(test_img, model params))
    ...
    list_of_models = [ResNet18().to(device) for _ in range(bundle_size)]
    models = ml.NetList(list_of_models)

    # Initialize the influence vector and Hessian matrix
    infl = [0*ntr]
    hessian = []
    # Inference Phase
    # Load information from A[t]
    output_path = "temp"
    fn = '%s/epoch%02d_info.dat' % (output_path, target_epoch)
    info = joblib.load(fn)
    T = len(info)
    B = [bundle_size] 
    B.append(T % (bundle_size))
    c = -1
    for k in range(1, -1, -1):
        fn = '%s/epoch%02d_bundled_models%02d.dat' % (output_path, target_epoch, k)
        models.load_state_dict(torch.load(fn))
        for t in tqdm(range(B[k]-1,-1,-1)):
            #find theta[t]
            theta_t= models.models[t].to(device)
            idx,lr,train_points = info[c]['idx'], info[c]['lr'], info[c]['train_points']
            c -=1
            # train_points = train_points.to(device)
            
            # Calculate the Hessian matrix H[t]
            #calculate the gradient of S_t and theta_t
            grads = grad_z(train_points[0],train_points[1],theta_t,gpu = 0)
            #calculate the hessian
            hessian = []
            torch.cuda.empty_cache()
            for grad in grads:
                hessian.append(torch.autograd.grad(grad.sum(), list(theta_t.parameters()), retain_graph=True))
            #convert to matrix
            H = torch.cat([h.view(-1) for h in hessian]).view(len(hessian), -1)
            print(H.shape)
            
            index_train = -1
            for train_point in train_points:
                #find the train_point in all_data
                index_train = data.index(train_point)
                #check
                print(index_train)
                #calculate the index
                influence_j = grad_z(train_point[0],train_point[1],theta_t)
                for i in range(t,T,1):
                    influence_j -= H @ influence_j

            infl[index_train] = influence_j
