...
def s_test(z_test, t_test, model, all_data, gpu=0, damp=0.01, scale=25.0,
    ...
    ################################
    data,label = all_data
    index = np.random.choice(len(data),recursion_depth)
    a_list = []
    for i in range(recursion_depth):
        ...
        #########################
        x,t = data[index[i]].cuda(),torch.tensor(label[index[i]]).cuda()
        # print(x.size())
        y = model(x)
        loss = calc_loss(y, t)
        ...
        a = (torch.norm(torch.nn.utils.parameters_to_vector(tuple(h_estimate))- torch.nn.utils.parameters_to_vector(tuple(h_estimate_copy)),2).item())
        if a<1.5e-5:
            break;
        else:
            h_estimate_copy = h_estimate.copy()
