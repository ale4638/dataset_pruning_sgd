#!/usr/bin/env python
"""Run Herding pruning experiments across datasets, pruning ratios, and seeds.

Herding selects samples closest to class cluster centers using greedy selection.
It requires extracting features from a randomly initialized model first.

Example usage
-------------
# All three datasets, default 3 seeds:
python scripts/run_herding.py --config configs/herding.yaml
...
from engine.trainer import train_and_evaluate, build_model
from engine.utils import save_json, seed_worker, set_seed, setup_logger
from pruning import HerdingPruningStrategy
...
def run_single(
    ...
    herding_strategy: HerdingPruningStrategy = None,
) -> dict:
    """Select subset via Herding → train from scratch → evaluate → save results."""
    ...
    # --- subset selection via Herding ---
    targets = get_targets(train_dataset)
    indices = herding_strategy.select_indices(
    ...
    lgr.info("=" * 60)
    lgr.info("Dataset: %s | Method: Herding | Prune: %.1f%% | Seed: %d",
...
def parse_args():
    p = argparse.ArgumentParser(description="Herding Pruning Experiments")
    p.add_argument("--config", type=str, default="configs/herding.yaml",
...
def main():
    ...
    # Herding-specific settings
    feature_batch_size = cfg["pruning"].get("feature_batch_size", 256)
    feature_num_workers = cfg["pruning"].get("feature_num_workers", 4)
    ...
    for ds_name in dataset_names:
        ...
        # --- Create feature extraction model (random init) ---
        # Use a fixed seed for the feature extraction model to ensure
        # reproducible features across different pruning ratios
        print("[herding] Creating feature extraction model (random init)...", flush=True)
        set_seed(42, deterministic=True)
        feature_model = build_model(num_classes).to(device)
        feature_model.eval()

        # Create Herding strategy with this model
        herding_strategy = HerdingPruningStrategy(
            model=feature_model,
            device=device,
        ...
        # Extract features once for all pruning ratios (cached in strategy)
        print("[herding] Pre-extracting features...", flush=True)
        targets = get_targets(train_ds)
        _ = herding_strategy.select_indices(
            dataset_size=len(train_ds),
            pruning_ratio=0,  # Just to trigger feature extraction
            seed=0,
            targets=targets,
            num_classes=num_classes,
            dataset=train_ds,
        )
        print("[herding] Features cached.", flush=True)
        ...
        for pr in ds_cfg["pruning_ratios"]:
            for seed in seeds:
                print(
                    f"\n>>> Starting: {ds_name} | method=herding | prune={pr}% | seed={seed}",
                    flush=True,
                )
                result = run_single(
                    ...
                    herding_strategy=herding_strategy,
                )
        ...
        # Clear feature cache before next dataset
        herding_strategy._features_cache = None

    save_json(all_results, str(Path(output_root) / "all_results.json"))
    print(f"\nAll results saved to {output_root}/")
    print("Run  python scripts/summarize_results.py --results_dir results/herding  to aggregate.")
